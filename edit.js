...
export default function Edit( { attributes, setAttributes } ) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Grid Settings', 'book-grid-block-wp' ) }
				>
					<RangeControl
						label={ __(
							'Columns (Desktop)',
						...
						onChange={ ( v ) =>
							setAttributes( { columnsDesktop: v } )
						}
			...
			<div { ...blockProps }>
				<div className="book-grid__controls aligned-controls">
					...
					>
						{ loading
							? __( 'Fetching…', 'book-grid-block-wp' )
							: __( 'Add Book', 'book-grid-block-wp' ) }
					</Button>
				</div>
				<div
					className="book-grid"
				...
				>
					{ books.length === 0 && (
						<p className="book-grid__placeholder">
							{ __(
								'No books added. Add book titles above to get started!',
								'book-grid-block-wp'
							) }
						</p>
					) }
			...
			{ detailsIdx !== null && (
				...
				>
					<TextControl
						...
						onChange={ ( v ) =>
							setDetailsDraft( { ...detailsDraft, caption: v } )
						}
						maxLength={ 160 }
						placeholder={ __(
							'Enter a caption…',
					...
					<TextControl
						...
						onChange={ ( v ) =>
							setDetailsDraft( { ...detailsDraft, link: v } )
						}
						placeholder="https://"
						__next40pxDefaultSize
						__nextHasNoMarginBottom
					...
					<div style={ { display: 'flex', gap: 8, marginTop: 16 } }>
						<Button variant="primary" onClick={ saveDetails }>
							{ __( 'Save', 'book-grid-block-wp' ) }
						</Button>
						<Button onClick={ closeDetails }>
							{ __( 'Cancel', 'book-grid-block-wp' ) }
						</Button>
					</div>
				</Modal>
			) }
